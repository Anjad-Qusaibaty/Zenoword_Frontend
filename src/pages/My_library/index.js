...
export default function Mylibrary() {
  ...
  // const [isBookTitle, setIsBookTitle] = useState(false);
  ...
  function compare(x, y) {
    const split_x = x
      .split(" ")
      .map((sx) => sx.toLowerCase().replace(/[&\\#,+()$~%.'":*?<>{}]/, ""));
    const split_y = y
      .split(" ")
      .map((sy) => sy.toLowerCase().replace(/[&\\#,+()$~%.'":*?<>{}]/, ""));
    let arr = [];
    for (let i = 0; i <= split_y.length; i++) {
      if (split_x.includes(split_y[i])) {
        arr.push(true);
      } else {
        arr.push(false);
      }
  ...
  function handel(event) {
    ...
    const filtering = user.extracts.filter(
      (ext) =>
        ...
        // ext.subtitle.toLowerCase().includes(search.toLowerCase()) ||
        // ext.tags.toLowerCase().includes(search.toLowerCase())

        compare(ext.text, search) ||
        compare(ext.author, search) ||
        compare(ext.title, search) ||
        compare(ext.subtitle, search) ||
        compare(ext.tags, search)
    );
  ...
  return (
    <div>
        ...
        <div className="bootstrapcheck">
          {/* <Form.Group controlId="formBasicCheckbox">
            <Form.Check
              ...
              label="Book title"
              value={isBookTitle}
              onChange={(event) => setIsBookTitle(!isBookTitle)}
            />
          </Form.Group> */}
        </div>
        <button className="newaddition" onClick={redirectToCreate}>
          + New Addition
        </button>
      </div>
      <div className="searchinfo">
        <p className="infoP">
          {" "}
          <span style={{ color: "#54CC82" }}>Search criteria: </span>
          {searchCriteria}
        </p>
        <p className="infoP">
          {" "}
          <span style={{ color: "#54CC82" }}>Search results: </span>
          {render.length}
        </p>
        <button
          onClick={reset}
      ...
      <div>
        {user.extracts.length > 0 && render.length === 0 ? (
          <p className="noresults">
            Sorry no results match your search criteria
          </p>
        ) : user.extracts.length === 0 ? (
          <p className="noresults">You have no extracts yet!</p>
        ) : null}
      </div>
      <div>
            ...
            <div key={Math.random()} className="allpage">
              <div className="container1">
                ...
                <div className="text">
                  ...
                  <div className="tagscont">
                    {anExtract.tags.length > 0
                      ? anExtract.tags.split(/[,;.]+/).map((tag) => (
                          <div className="extractTag" key={Math.random()}>
                            #{tag}
                          </div>
                        ))
                      : null}
                  ...
                  <div>
                    <a href={`/edit/${anExtract.id}`} className="edit">
                      <i className="far fa-edit"></i>
                    </a>
                    <a href={`/delete/${anExtract.id}`} className="delete">
